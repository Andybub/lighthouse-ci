  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _updateImages: function (variant) {
      if ($('.color_sw.selected .swatch').length != 0) {
        var firstVariantImage = $('.color_sw.selected .swatch')
      ...
      if (
        (!variant.featured_media && firstVariantImage == undefined) ||
        (variantImage.src === currentVariantImage && firstVariantImage == undefined)
      ) {
        return;
      }
      this.$container.trigger({
...
theme.Product = (function () {
  function Product(container) {
    ...
    if (this.dataSettings.useMaxheight) {
      if (
        $(self.selectors.template + ' iframe#resize-image-product').length > 0 ||
        $(self.selectors.template + ' iframe#resize-image-product-qv').length > 0
      ) {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _initSwatch: function () {
      ...
      $(self.$container).on('click', '.--js-opt', function (e) {
        ...
        var selector = self.selectors.template + ' .--js-selector[data-js-opt="' + optionIndex + '"]',
          $selector = $(selector);
        ...
        $(self.selectors.cartForm + ' #SglOptSelector-' + optionIndex)
          .val(optionValue)
          .trigger('change');
    ...
    _initVariants: function () {
      ...
      if (!this.dataJs.dfVariant) {
        ...
        var option = this.variants.currentVariant.option1,
          check_option = 0;
        ...
        _.forEach(self.productSingleObject.variants, function (variant) {
          $.each($('#' + optitonSelector1 + ' option'), function (index, el) {
            ...
            if (check_option == 0 && $(this).val() == variant.option1) {
              $(this).attr({
                'data-first-variant': $(self.selectors.originalSelectorId + ' option[value="' + variant.id + '"]').data(
                  'value',
                ),
              });
        ...
        $.each($('.sgl-opt-selector-' + self.settings.sectionId), function (index, el) {
          ...
          if (
            settings_op.style === 'combobox with_out_variant_image' ||
            (settings_op.color_watched && self.dataSettings.swatchStyle !== 'color')
          ) {
            $.each($selector.find('.--js-opt'), function (idx, el) {
              var $this_ = $(this);
              var featuredImage = $this
                .find('option[value="' + $this_.attr('data-value') + '"]')
                .attr('data-featured-image');
              if (featuredImage === undefined) {
                return;
              }
              if (settings_op.style === 'combobox with_out_variant_image') {
                var fileExt = _.last(featuredImage.split('.')),
                  featuredImage = featuredImage.replace('.' + fileExt, '_150x150_crop_center.' + fileExt);
                $(
                  '[data-n-opt="' +
                    $selector.data('n-opt') +
                    '"] .--js-opt[data-value="' +
                    $this_.attr('data-value') +
                    '"]',
                )
                  .find('img')
                  .attr('data-src', featuredImage)
                  .addClass('lazyload');
              } else if (settings_op.color_watched && self.dataSettings.swatchStyle !== 'color') {
                var fileExt = _.last(featuredImage.split('.')),
                  featuredImage = featuredImage.replace('.' + fileExt, '_100x100.' + fileExt);
                $(
                  '[data-n-opt="' +
                    $selector.data('n-opt') +
                    '"] .--js-opt[data-value="' +
                  ...
                  .find('.swatch')
                  .css({ 'background-image': 'url("' + featuredImage + '")', 'background-color': 'transparent' });
              }
            });
          }
        });
    ...
    _AjaxForm: function () {
      ...
      $form.on('change', function (event) {
        ...
        if (itemsGroup !== itemGroup.length) {
          ...
          $('.subtt_gr--' + self.settings.sectionId + ' .subtt_gr-money').html(
            theme.Currency.formatMoney(crr_pr, theme.moneyFormat),
          );
          if (crr_pr !== crr_c_pr) {
            $('.subtt_gr--' + self.settings.sectionId + ' .subtt_gr-c-money')
              .html(theme.Currency.formatMoney(crr_c_pr, theme.moneyFormat))
              .removeClass('d-none');
          } else {
      ...
      if (this.settings.ajaxCart) {
        $(document).on('submit', self.selectors.cartForm, function (e) {
          ...
          if ($form.find('.itemGroupProducts').length > 0) {
            ...
            console.log('Shopify.KT_addItems', queue);
            Shopify.KT_addItems(queue);
          } else {
            console.log('Shopify.KT_addItemFromForm');
    ...
    _initVariant: function (currentVariant) {
      ...
      var options2 = new Array(),
        options3 = new Array();
      var option = currentVariant.option1,
        check_option = 0;
      var optitonSelector1 = 'SglOptSelector-0--' + self.settings.sectionId;
      var optitonSelector2 = 'SglOptSelector-1--' + self.settings.sectionId;
      var optitonSelector3 = 'SglOptSelector-2--' + self.settings.sectionId;
          ...
          if (value.option1 == currentVariant.option1 && value.option2 == currentVariant.option2) {
            $.each($('#' + optitonSelector3 + ' option'), function (index, el) {
              if (value.option3 == $(this).attr('value')) {
                ...
                $('[data-js-opt="2--' + self.settings.sectionId + '"] [data-value="' + value.option3 + '"]').addClass(
                  '_soulout',
                );
            ...
            $.each($('#' + optitonSelector2 + ' option'), function (index, el) {
              if (value.option2 == $(this).attr('value')) {
                // $(this).text($(this).text()+' - Soldout');
                $('[data-js-opt="1--' + self.settings.sectionId + '"] [data-value="' + value.option2 + '"]').addClass(
                  '_soulout',
                );
                // .find('.kt_flash_lable').text(' - Soldout');
              }
      ...
      $.each($('#' + optitonSelector2 + ' option'), function (index, el) {
        if (_.indexOf(options2, $(this).attr('value')) == -1) {
          $('[data-js-opt="1--' + self.settings.sectionId + '"] [data-value="' + $(this).val() + '"]')
            .addClass('_unavailable')
            .find('.kt_flash_lable')
            .text(' - Unavailable');
        } else if (self.dataSettings.swatchStyle !== 'color') {
          var variant = _.filter(self.productSingleObject.variants, {
          ...
          if (featuredImage !== undefined) {
            var fileExt = _.last(featuredImage.split('.')),
              featuredImage = featuredImage.replace('.' + fileExt, '_100x100_crop_center.' + fileExt);
            $('[data-js-opt="1--' + self.settings.sectionId + '"] [data-value="' + $(this).val() + '"] .swatch').css(
              'background-image',
              'url("' + featuredImage + '")',
            );
          }
      ...
      $.each($('#' + optitonSelector3 + ' option'), function (index, el) {
        if (_.indexOf(options3, $(this).attr('value')) == -1) {
          $('[data-js-opt="2--' + self.settings.sectionId + '"] [data-value="' + $(this).val() + '"]')
            .addClass('_unavailable')
            .find('.kt_flash_lable')
            .text(' - Unavailable');
        } else if (self.dataSettings.swatchStyle !== 'color') {
          ...
          if (featuredImage !== undefined) {
            var fileExt = _.last(featuredImage.split('.')),
              featuredImage = featuredImage.replace('.' + fileExt, '_100x100_crop_center.' + fileExt);
            $('[data-js-opt="2--' + self.settings.sectionId + '"] [data-value="' + $(this).val() + '"] .swatch').css(
              'background-image',
              'url("' + featuredImage + '")',
            );
          }
    ...
    _initImageSwitch: function () {
      ...
      $(self.$container).on('click', this.selectors.productThumbImages, function (evt) {
        ...
        if (typeof $el.attr('data-id') !== 'undefined' && $el.attr('data-id') !== '') {
          var varaintsByMedia = _.filter(self.productSingleObject.variants, function (variant) {
            if (variant.featured_image && variant.featured_image.variant_ids.includes(parseInt($el.attr('data-id')))) {
              return variant;
            }
          });
          if (varaintsByMedia.length > 1) {
            $(template + ' .--js-opt[data-value="' + varaintsByMedia[0].option1 + '"]')
              .first()
              .trigger('click');
          } else {
            goto = false;
            ...
            $(varaintsByMedia[0].options).each(function (index, opiton) {
              ...
              if (
                opiton != self.productSingleObject.title &&
                $('[data-js-opt="' + index + '--' + self.settings.sectionId + '"]').hasClass('op_pre_select') === false
              ) {
                $(
                  template +
            ...
            if (
              $(this).parents('div.product-page').data('enable-history-state') == true &&
              _.filter(theme.function.productOptionStyle, { op_pre_select: true }).length <= 0
            ) {
              var newurl =
        ...
        options.on = {
          ...
          slideChange: function (swiper) {
            ...
            if (swiper.thumbs.swiper) {
              ...
              if (
                crridx === thumbsSwiper.visibleSlidesIndexes[thumbsSwiper.visibleSlidesIndexes.length - 1] &&
                crridx < slides
              ) {
                self.thumbSlider.translateTo(
                  -1 * thumbsSwiper.snapGrid[thumbsSwiper.visibleSlidesIndexes[1]],
                  300,
                  false,
                );
              }
              if (crridx === thumbsSwiper.visibleSlidesIndexes[0] && crridx !== 0) {
                self.thumbSlider.translateTo(
                  -1 * thumbsSwiper.snapGrid[thumbsSwiper.visibleSlidesIndexes[0] - 1],
                  300,
                  false,
                );
              }
            ...
            if (itemCrr.find('.plyr__video-wrapper video').length > 0) {
              ...
              $(self.selectors.template + ' .view-in-space').attr(
                'data-shopify-model3d-id',
                itemCrr.find('model-viewer').attr('data-model-id'),
              );
            }
            if (theme.is_mobile == false && self.settings.zoomEnabled && typeof $.fn.zoom == 'function') {
              ...
              $.each(
                $(self.selectors.template + ' .photos-wrapper .gallery-image.swiper-slide-visible'),
                function (index, val) {
    ...
    _mediaSlider: function (autoplay) {
      var self = this;
      if ($(self.selectors.productPhotosWrapper + ' .plyr__video-wrapper video').length > 0) {
        return true;
      }
      if ($(self.selectors.productPhotosWrapper + ' .bgndVideo.mb_YTPlayer').length > 0) {
        return true;
      }
      if ($(self.selectors.productPhotosWrapper + ' .shopify-model-viewer-ui').length > 0) {
        return true;
      }
    ...
    _updateVariant: function (evt) {
      ...
      if ($('.prd-mirror-' + this.productSingleObject.id).length > 0) {
        $('.prd-mirror-' + this.productSingleObject.id)
          .val(variant.id)
          .trigger('change');
      }
      this._initVariant(variant);
      if (variant.featured_media == null || variant.featured_media == undefined) {
        return;
      }
      if (this.dataSettings.gallery && dataJs.curpos.join(',') != dataJs.imgspos) {
        if (this.currentMedia.src == variant.featured_media.preview_image.src) {
          return;
        }
    ...
    _updateAddToCart: function (evt) {
      ...
      if (variant) {
        ...
        if ($(template + ' .notify-available [name="contact[body]"]').length > 0) {
          $(template + ' .notify-available [name="contact[body]"]').val(
            $(template + ' .notify-available [name="contact[body]"]')
              .val()
              .replace(/\(.*\)/g, '(' + variant.title + ')'),
          );
        }
        ...
        if (
          $('#SglOptSelector-0--' + sectionId) !== undefined &&
          $('[data-js-opt="0--' + sectionId + '"]').hasClass('op_pre_select') == false
        ) {
          $('label[data-change-label="0--' + sectionId + '"] .js-change-label').html(
            $('#SglOptSelector-0--' + sectionId).val(),
          );
        }
        if (
          $('#SglOptSelector-1--' + sectionId) !== undefined &&
          $('[data-js-opt="1--' + sectionId + '"]').hasClass('op_pre_select') == false
        ) {
          $('label[data-change-label="1--' + sectionId + '"] .js-change-label').html(
            $('#SglOptSelector-1--' + sectionId).val(),
          );
        }
        if (
          $('#SglOptSelector-2--' + sectionId) !== undefined &&
          $('[data-js-opt="2--' + sectionId + '"]').hasClass('op_pre_select') == false
        ) {
          $('label[data-change-label="2--' + sectionId + '"] .js-change-label').html(
            $('#SglOptSelector-2--' + sectionId).val(),
          );
        }
        ...
        if (variant.available) {
          if (self.dataSettings.useQty) {
            $(template)
              .find('.item-quantity--' + sectionId)
              .show();
          }
          $(template).find('.kt_progress_bar').slideDown();
          $('.fbt-item #ip_' + self.productSingleObject.id + ':not(:checked)')
            .removeAttr('disabled')
            .trigger('click')
          ...
          $('.fbt-item #ip_' + self.productSingleObject.id + ':checked')
            .removeAttr('disabled')
            .trigger('click')
            .attr('disabled', 'disabled');
          $(template)
        ...
        if (variant.available || variant.incoming) {
          ...
          if (
            (variant.inventory_quantity <= 0 && variant.inventory_management === 'shopify' && variant.incoming) ||
            (!variant.inventory_quantity && variant.incoming)
          ) {
            $('.preOrder--' + sectionId)
              .slideDown()
              .find('span')
              .html(variant.incoming);
        ...
        if (goto == true) {
          $('[data-op-pre-select]').each(function (index, el) {
            ...
            $this
              .find('.js-change-label')
              .html(theme.productStrings.label_select + $this.attr('data-n-opt').toLowerCase());
          });
          var option1 = $('#SglOptSelector-0--' + sectionId).val();
          var option1_cr = option1;
          console.log(option1, $('#SglOptSelector-0--' + sectionId + ' option[value="' + option1 + '"]'));
          var options = $('#SglOptSelector-0--' + sectionId + ' option[value="' + option1 + '"]')
    ...
    _updateImages: function (evt) {
      ...
      if (variantImage != undefined && goto == true) {
        var $photo = $(
          this.selectors.productPhotos +
            ' .p-sgl__photos-item[data-src="' +
            theme.Images.removeProtocol(variantImage) +
            '"]',
        );
    ...
    _updateGallery: function (variant) {
      var self = this;
      var dataJs = this.dataJs;
      var imgPos = variant.featured_media
        ? _.indexOf(_.map(self.previewImage, 'src'), variant.featured_media.preview_image.src) + 1
        : parseInt($(this.selectors.dataJs).attr('data-vrimgpos'));
      var imgspr = dataJs.imgspos.split(','),
        imgsArray = _.map(self.previewImage, 'src');
      var htmlPhotoImages = '';
      if ($(self.selectors.productThumbImages).length >= 1) {
        var htmlThumbImages = '';
      }
      var last_imgPos = $(this.selectors.dataJs).attr('data-vrimgpos');
      if (_.indexOf(imgspr, imgPos.toString()) == -1 && last_imgPos == imgPos.toString()) {
        return true;
      }
      var imgsPos = [];
      ...
      if (imgspr.length <= 0 && !newgallery) {
        return true;
      }
      $(this.selectors.dataJs).attr('data-curpos', '[' + imgsPos[0] + ',' + imgsPos[1] + ']');
      $(this.selectors.dataJs).attr('data-vrimgpos', imgPos);
      var imgsprw = _.map(self.previewImage, 'width'),
        imgsprh = _.map(self.previewImage, 'height');
      if ($(this.selectors.productThumbs).length > 0) {
        var image_size =
          theme.Images.imageSize($(this.selectors.productThumbs + ' .p-sgl__thumb-image:first').attr('src')) ||
          theme.Images.imageSize($(this.selectors.productThumbs + ' .p-sgl__thumb-image:first').attr('data-src'));
      }
      ...
      _.forEach(self.productSingleObject.media, function (media, idx) {
        ...
        if (
          parseInt(imgsPos[0]) == parseInt(imgsPos[1]) &&
          parseInt(imgsPos[0]) == idx + 1 &&
          parseInt(imgsPos[1]) == imgsArray.length
        ) {
          kt_visible = true;
        }
        if (
          imgspr[imgspr.length - 2] !== imgspr[imgspr.length - 1] &&
          parseInt(imgsPos[1]) == idx + 1 &&
          parseInt(imgsPos[1]) == imgsArray.length
        ) {
          kt_visible = true;
        }
        if (kt_visible == true) {
          var fileExt = _.last(media.preview_image.src.split('.')),
            img_url_photo = media.preview_image.src.replace('.' + fileExt, '_{width}x.' + fileExt);
          if ($(self.selectors.productThumbs).length > 0) {
            var img_url_thumb = media.preview_image.src.replace('.' + fileExt, '_' + image_size + '.' + fileExt);
          }
          var varaintsByMedia = _.filter(self.productSingleObject.variants, function (variant) {
            if (variant.featured_media && variant.featured_media.preview_image.src == media.preview_image.src) {
              return variant;
            }
          });
          ...
          var image_fit =
            theme.function.productImgType === 'stretch'
              ? theme.Images.imgFit(media.preview_image.width, media.preview_image.height, a_ratioW, a_ratioH)
              : '';
          ...
          htmlPhotoImages +=
            '<div class="swiper-slide p-sgl__photos-item  gallery-' +
            media.media_type +
            productImgType +
            positionCss +
            '" data-src="' +
            ...
            '">';
          htmlPhotoImages +=
            '<div class="p-sgl__photo p-sgl__photo-' +
            idx +
            ' p-sgl__photo--' +
            self.settings.sectionId +
            ' aspectRatio" data-src="' +
            media.preview_image.src +
            '"' +
            alt +
            id +
          ...
          if (media.media_type !== 'external_video') {
            htmlPhotoImages +=
              ...
              '" class="p-sgl__photo-image lazyload mll-lz ' +
              image_fit +
              '"';
            htmlPhotoImages += ' src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="';
            htmlPhotoImages +=
              ' data-srcbase="' +
              media.preview_image.src.replace('.' + fileExt, '_' + image_size_ + '.' + fileExt) +
              '"';
            ...
            htmlPhotoImages +=
              ' data-sizes="auto" style="padding-bottom: ' + (1.0 / media.aspect_ratio) * 100.0 + '%">';
          }
          switch (media.media_type) {
            case 'external_video':
              ...
              if (media.host == 'youtube') {
                htmlPhotoImages +=
                  '<div class="bgndVideo" data-property="{videoURL:\'https://www.youtube.com/watch?v=' +
                  media.external_id +
                  "', quality:'highres', stopMovieOnBlur: false, containment: 'self', vol: 100}\"></div>";
              } else {
                htmlPhotoImages +=
                  '<iframe src="https://player.vimeo.com/video/' +
                  media.external_id +
                  '?autoplay=1&loop=1&title=0&byline=0&portrait=0" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>';
              }
              htmlPhotoImages += '</div>';
              htmlPhotoImages += '</div>';
              htmlPhotoImages += '</div>';
            ...
            case 'video':
              ...
              htmlPhotoImages +=
                '<video playsinline="playsinline" controls="controls" loop="loop" preload="none" class="video-element" aria-label="' +
                self.productSingleObject.title +
                '" poster="' +
                media.preview_image.src.replace('.' + fileExt, '_720x.' + fileExt) +
                '">';
              $.each(media.sources, function (index, source) {
                htmlPhotoImages += '<source src="' + source.url + '" type="' + source.mime_type + '">';
              });
              htmlPhotoImages +=
                '<img src="' + media.preview_image.src.replace('.' + fileExt, '_720x.' + fileExt) + '">';
            ...
            case 'model':
              ...
              htmlPhotoImages +=
                '<model-viewer reveal="interaction" toggleable="true" data-model-id="' +
                media.id +
                '" src="' +
                media.sources[0].url +
                '" camera-controls="true" data-shopify-feature="0.8" alt="' +
                self.productSingleObject.title +
                '" poster="' +
                media.preview_image.src.replace('.' + fileExt, '_720x.' + fileExt) +
                '"></model-viewer>';
          ...
          if (theme.function.productImgType === 'normal' && self.settings.imagesLayout === 'masonry') {
            htmlPhotoImages +=
              '<style media="(min-width: 992px)">.product-page .aspectRatio.p-sgl__photo-' +
              idx +
              '{padding-bottom:' +
              (1 / media.preview_image.aspect_ratio) * 100.0 +
              '%}</style>';
          }
          htmlPhotoImages += '</div></div>';
          if ($(self.selectors.productThumbs).length > 0) {
            htmlThumbImages +=
              '<div class="swiper-slide p-sgl__thumbs-item gallery-' +
              media.media_type +
              productImgType +
              '" data-src="' +
              media.preview_image.src +
              '">';
            htmlThumbImages +=
              '<div class="p-sgl__thumb p-sgl__thumb--' +
              self.settings.sectionId +
              ' aspectRatio" data-src="' +
              media.preview_image.src +
              '"' +
              ...
              '>';
            htmlThumbImages +=
              '<img class="p-sgl__thumb-image lazyload mll-lz ' +
              image_fit +
      ...
      if (self.settings.imagesLayout === 'masonry' && theme.window_W >= 992) {
          ...
          if (index + 1 == $(self.selectors.productPhotos + ' .p-sgl__photos-item').length) {
            ...
            $.each($(self.selectors.productPhotos + ' .p-sgl__photos-item'), function (index, val) {
              if ($(this).attr('style') == undefined) {
                $(self.selectors.productPhotos).isotope('appended', $(this)).isotope('layout');
              }
              if (index + 1 == $(self.selectors.productPhotos + ' .p-sgl__photos-item').length) {
                self._mediaSlider(false);
              }
        ...
        var thumbs = $('#product-thumb-slide--' + self.settings.sectionId),
          photos = $('#product-photo-slide--' + self.settings.sectionId);
    ...
    _updatePrice: function (evt) {
      ...
      $(this.selectors.originalPrice.replace('#', '.')).html(
      ...
      if ($('.fbt-item .ktlz-form-pid-' + self.productSingleObject.id).length > 0) {
        $('.fbt-item .ktlz-form-pid-' + self.productSingleObject.id + ' ins').html(
          theme.Currency.formatMoney(variant.price, theme.moneyFormat),
        );
        if (variant.compare_at_price > variant.price) {
        ...
        KT.loadScript('zoom', function (e, l) {
          if (l == 'ok') {
            ...
            $(self.$container).on('click', 'img.zoomImg', function (event) {
              event.preventDefault();
              $(
                self.selectors.template +
                  ' img[data-srcfix="' +
                  theme.Images.removeProtocol(event.target.currentSrc) +
                  '"]',
              ).trigger('click');
            });
          }
    ...
    _initPhotoSwipeFromDOM: function () {
      ...
      if (self.dataSettings.wide) {
        KT.loadScript('photo-swipe', function (e, l) {
          if (l == 'ok') {
            theme.initPhotoSwipeFromDOM(
              self.selectors.template + ' .photos-wrapper',
              '.p-sgl__photo',
              'IMG',
              function (currentIndex, currentItem) {
                if (
                  $(self.selectors.template + ' .photos-wrapper .p-sgl__photos-item').length !== 0 &&
                  self.photoSlider !== undefined
                ) {
                  var itemSlick = $(self.selectors.template + ' .photos-wrapper').find(
                    '.p-sgl__photos-item[data-src~="' + currentItem.src + '"]',
                  );
    ...
    _stickyAddCart: function () {
      ...
      var stickyAddCart = function () {
        ...
        if (
          scrollTop > $(self.selectors.addToCart).offset().top + 60 &&
          $('.kt-stickyAddCart').hasClass('fixed') === false
        ) {
    ...
    _initStickySummary: function () {
      ...
      var thisposition_max =
        scrollTop + thissummary[0].getBoundingClientRect().top + (thissummary.outerHeight() - thissummary.height());
      function sticky() {
        var $window = $(window);
        if (theme.window_W < 992) {
          return;
        }
        ...
        if (mirrorHeight > scrollTop + thissummaryh - thisposition_max) {
          if (scrollTop + theme.window_H - 200 >= thisposition && thissummary.hasClass('fixed') == false) {
            ...
            thissummary
              ...
              .css({ top: transalte + 'px', width: thissummaryw + 'px', transform: 'none' });
          ...
          thissummary.hasClass('maxscroll') == false &&
          mirrorHeight <= thissummaryh + thissummary.offset().top
        ) {
          ...
          thissummary
            ...
            .removeAttr('style')
            .css({
              transform: 'translate3d(0px,' + transalte + 'px,0px)',
              '-webkit-transform': 'translate3d(0px,' + transalte + 'px,0px)',
              'moz-transform': 'translate3d(0px,' + transalte + 'px,0px)',
              '-ms-transform': 'translate3d(0px,' + transalte + 'px,0px)',
              '-o-transform': 'translate3d(0px,' + transalte + 'px,0px)',
            });
        }
      }
      function transalte() {
    ...
    _onReSizeInfo: function (refresh) {
      ...
      function sticky() {
        ...
        if (mirrorHeight > scrollTop + thissummaryh - thisposition_max) {
          if (
            scrollTop + theme.window_H - 200 >= thisposition &&
            $('.summary.entry-summary').hasClass('fixed') == false
          ) {
            var transalte = $('header[data-header-sticky="true"]').length > 0 ? 85 : 20;
            thissummary
              .not('.fixed')
    ...
    _create360: function (variant) {
      var dataJs = this.dataJs,
        self = this;
      // Check 360 available
      var imgsPos360 = dataJs.imgspos360.split(','),
        imgsArray = _.map(self.previewImage, 'src'),
        imgsprw = _.map(self.previewImage, 'width'),
        imgsprh = _.map(self.previewImage, 'height');
      var files = [];
    ...
    _shipTo: function () {
      function buildRates(data) {
        ...
        var list_shipping = '';
        var data = JSON.parse(
          data.replace('<!-- BEGIN template --><!-- cart.listShipping -->', '').replace('<!-- END template -->', ''),
        );
        $('.shipByLocation .flag').html(
          '<img width="26" height="19.5" class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="' +
            data.flag +
            '" alt="' +
            data.code +
            '">',
        ...
        if ($('.shipping_rates .shipByLocation').length <= 0) {
          return;
        }
        $('.shipping_rates').show();
      }
    ...
    _stockCountdown: function () {
      ...
      function updateMeter(container, remaining_items_, important) {
        ...
        var content =
          theme.productStrings.stockMessage[0] +
          ' <span class="count">' +
          remaining_items_ +
          '</span> ' +
          theme.productStrings.stockMessage[1];
    ...
    _realTimeVisitor: function (el) {
      ...
      $(function (a) {
        ...
        setInterval(function () {
          ...
          $el.find('#number_counter>span').html(parseInt(o));
          $el.show();
        ...
        KT.loadScript('momentlocale', function (e, l) {
          if (l == 'ok') {
            ...
            if ($('.product_delivery .timeDay').length > 0) {
              ...
              $el.append(
                '<style>span.timeDay{display: inline-block}.timeDay .block{display: inline-block}.timeDay span.label{padding: 0 4px}</style>',
              );
            ...
            $el.slideDown();
            if ($el.parents('.shipping_rates').length <= 0) {
              return;
            }
    ...
    _responTableGroup: function () {
      var self = this;
      $(self.selectors.template + ' .table-products').scroll(
